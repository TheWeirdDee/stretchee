...
export default function Hero() {
      ...
      gsap.to(studentsObj, {
        ...
        onUpdate: () => {
          if (studentCountRef.current) {
            studentCountRef.current.innerText = Math.floor(
              studentsObj.val,
      ...
      gsap.to(classesObj, {
        ...
        onUpdate: () => {
          if (classesCountRef.current) {
            classesCountRef.current.innerText =
              Math.floor(classesObj.val).toLocaleString("en-US") + "+";
          }
        },
        delay: 0.7,
      });
      ...
      gsap.to(satisfactionObj, {
        ...
        onUpdate: () => {
          if (satisfactionCountRef.current) {
            satisfactionCountRef.current.innerText =
              Math.floor(satisfactionObj.val) + "%";
          }
        },
        delay: 0.9,
      });
  ...
  return (
    <div
      ref={container}
      className="relative min-h-screen w-full flex flex-col font-sans text-white overflow-hidden bg-black"
    >
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <main className="flex-1 flex flex-col justify-center px-6 md:px-12 lg:px-16 mt-10 md:mt-20">
          <div className="max-w-2xl">
            <h1
              ref={textRef1}
              className="text-5xl md:text-6xl lg:text-[5rem] font-medium leading-[1.1] tracking-tight mb-6 opacity-0"
            >
              <span className="text-4xl md:text-6xl lg:text-[5rem]">
                Flow into <span className="opacity-70">—</span> Peace
              </span>
              <br />
              One Pose at a Time
            </h1>
            ...
            <p
              ref={textRef2}
              className="text-base md:text-lg max-w-sm md:max-w-md opacity-0 leading-relaxed mb-10 font-light tracking-wide"
            >
              Whether you're just beginning or mastering your journey, join our
              community for a rejuvenating path toward well-being.
            </p>
        ...
        <footer className="w-full px-6 md:px-12 lg:px-16 py-8 md:py-12 mt-auto">
          <div className="flex flex-col sm:flex-row gap-8 sm:gap-12 md:gap-20">
            <div className="stat-item flex flex-col gap-1 opacity-0">
              ...
              <span className="text-sm opacity-70 tracking-wide font-light">
                Happy Students
              </span>
            </div>
